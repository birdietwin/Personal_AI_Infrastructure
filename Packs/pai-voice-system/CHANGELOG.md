...
### Added
- **Voice Queuing System**: Notifications now queue and play sequentially with brief pauses
  - Added `QueuedNotification` interface and notification queue array
  - Added `processQueue()` function for sequential notification processing
  - Added `processNotificationImmediate()` for individual notification handling
  - Added 400ms "breath" pause between consecutive notifications
  - Prevents overlapping speech when multiple notifications arrive simultaneously

### Changed
- **Refactored `sendNotification()`**: Now queues requests instead of playing immediately
  - HTTP requests wait until their specific notification completes
  - Queue status logged to server console for visibility
  - Configurable pause duration via `PAUSE_BETWEEN_NOTIFICATIONS` constant

### Philosophy
- Sequential playback ensures each notification is clearly audible
- Brief pause mimics natural speech patterns between utterances
- Queue system maintains responsiveness while preventing audio overlap
